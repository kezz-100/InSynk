...
export default function News() {
    ...
    axios
      ...
      .then((result) => {
        ...
        if (data.statusCode === 200) {
          ...
          updateReservedIntervals(data.listNews);
        }
      })
      .catch((error) => {
        console.log(error);
  ...
  const updateReservedIntervals = (listNews) => {
    const reserved = listNews.map((news) => {
      const dueDateTime = new Date(news.dueDate);
  ...
  */
  ...
  const handleSave = (e) => {
    ...
    if (editingNewsId !== null) {
      ...
      const url = `https://localhost:7089/api/News/EditNews`;
      axios
        .put(url, updatedData)
        .then((result) => {
          const dt = result.data;
  ...
  const handleDelete = (id) => {
    ...
    axios
      ...
      .then((result) => {
        const dt = result.data;
        if (dt.statusCode === 200) {
          setNewsDeleted(true); // Set newsDeleted to true when news is deleted
          setTimeout(() => {
            setNewsDeleted(false); // Reset newsDeleted after a short delay
            getData();
