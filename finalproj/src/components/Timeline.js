...
export default function Timeline() {
  ...
  // Convert news data to scheduler appointments
  const convertToAppointments = (newsData, approvedUsers) => {
    return newsData.map((news) => {
      // Find the user object based on the assigned user ID
      const assignedUser = approvedUsers.find(
        (user) => user.id === parseInt(news.assignedUser, 10)
      );
      ...
      return {
        ...
        title: news.title,
        startDate: new Date(news.dueDate),
        endDate: new Date(news.dueDate),
        priority: news.priority,
        content: news.content,
        assignedUser: assignedUserEmail,
        isCompleted: news.isCompleted,
      };
    });
  };
  ...
  const schedulerData = convertToAppointments(newsData, approvedUsers);
    ...
    <div>
      ...
      >
        <Scheduler data={schedulerData}>
          ...
          {/* <Appointments
            appointmentComponent={({ style, ...restProps }) => (
              <Appointments.Appointment
                {...restProps}
                style={{ ...style, ...appointmentStyle(restProps.data) }}
                onClick={() => handleAppointmentClick(restProps.data)}
              >
                <div style={{ fontSize: "22px" }}>VIEW</div>
              </Appointments.Appointment>
            )}
